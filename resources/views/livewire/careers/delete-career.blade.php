<div class="ui tiny test modal" id="delete-career-{{ $career->id }}-modal" modal-status="uninitialized" wire:ignore.self>
	...
	<form class="actions" id="delete-career-{{ $career->id }}-form" wire:submit.prevent="destroyCareer" wire:reset="resetForm" wire:loading.class="loading">
		...
		<button class="ui red right labeled icon button" form="delete-career-{{ $career->id }}-form" type="submit" wire:loading.class="loading" wire:loading.attr="disabled" wire:target="updateArea">Sí, continuar<i class="arrow right icon"></i></button>
	</form>
</div>
