<div class="ui tiny test modal" id="delete-area-{{ $area->id }}-modal" modal-status="uninitialized" wire:ignore.self>
	...
	<form class="actions" id="delete-area-{{ $area->id }}-form" wire:submit.prevent="destroyArea" wire:reset="resetForm" wire:loading.class="loading">
		...
		<button class="ui red right labeled icon button" form="delete-area-{{ $area->id }}-form" type="submit" wire:loading.class="loading" wire:loading.attr="disabled" wire:target="updateArea">Sí, continuar<i class="arrow right icon"></i></button>
	</form>
</div>
