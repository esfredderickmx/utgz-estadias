			...
			<table class="ui selectable single line very basic celled table">
				<thead>
					<tr>
						...
						<th class="center aligned">Correo electrónico</th>
						<th class="center aligned">Teléfono</th>
						<th class="center aligned">Oportunidades</th>
						<th></th>
					</tr>
				</thead>
				<tbody>
					@forelse ($users as $user)
						<tr target-modal="info-user-{{ $user->id }}-modal">
							<td class="center aligned" data-tooltip="{{ $user->role === 'super' ? 'Superusuario' : ($user->role === 'admin' ? 'Administrativo' : ($user->role === 'manager' ? 'Jefe de área' : ($user->role === 'adviser' ? 'Asesor' : 'Estudiante'))) }}" data-position="bottom left" data-inverted>
                ...
                <div wire:loading.class="active" class="ui inline small loader"></div>
							...
							<td class="center aligned">{{ strtok($user->first_name, ' ') . ' ' . strtok($user->last_name, ' ') }}</td>
